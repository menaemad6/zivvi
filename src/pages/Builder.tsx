...
const { duplicateCv } = useDuplicateCV();
  ...
  const [currentTemplate, setCurrentTemplate] = useState('classicTemp');
  ...
  const handleDuplicate = () => {
    if (!id || id === 'new' || !cvData) {
      toast({
        title: "Save CV First",
        description: "Please save your CV before duplicating.",
        variant: "destructive"
      });
      return;
    }
    
    // Create a CV object with the required properties
    const cvToduplicate = {
      id,
      title: cvMetadata.name || 'My CV',
      name: cvMetadata.name || null,
      description: cvMetadata.description || null,
      template: currentTemplate,
      created_at: new Date().toISOString(),
      updated_at: new Date().toISOString()
    };
    
    duplicateCv(cvToduplicate);
  };
    ...
    <>
      ...
      <div className="flex min-h-screen bg-gradient-to-br from-violet-50 via-white to-cyan-50 pt-8 sm:pt-14 md:pt-12 ">
        ...
        <TemplateSelectionModal
          ...
          currentTemplate={currentTemplate || 'classicTemp'}
